...
/**
 * 複数引数の関数のメモ化（引数の組み合わせでキーを生成）
 */
export const memoizeMultipleArgs = <TReturn>(
  fn: (...args: unknown[]) => TReturn,
  options: MemoizationOptions = {}
): Result<(...args: unknown[]) => TReturn, MemoizationError> => {
  return memoize(fn, options);
};

// メモ化された関数の型定義
export interface MemoizedFunction<TArgs extends readonly unknown[], TReturn> {
  (...args: TArgs): TReturn;
  cache: {
    clear: () => void;
    ...
    size: () => number;
    entries: () => Array<[string, CacheEntry<TReturn>]>;
  };
}
...
export const getCacheStats = (
  memoizedFn: Function & { cache?: { size: () => number } }
): {
  size: number;
  maxSize: number;
} | null => {
  if (!memoizedFn.cache) return null;

  return {
    size: memoizedFn.cache.size(),
    maxSize: 100, // デフォルト値、実際の設定値を取得する場合は追加実装が必要
  };
};
...
export const clearAllCaches = (
  memoizedFunctions: readonly (Function & { cache?: { clear: () => void } })[]
): void => {
...
/**
 * TTL付きメモ化の便利関数（5分間キャッシュ）
 */
export const memoizeWithExpiration = <
  TArgs extends readonly unknown[],
  TReturn
>(
  fn: (...args: TArgs) => TReturn,
  expirationMs: number = 5 * 60 * 1000 // 5分
): Result<(...args: TArgs) => TReturn, MemoizationError> => {
  return memoize(fn, {
    ttl: expirationMs,
    maxSize: 50,
...
/**
 * 重い計算用のメモ化（大きなキャッシュサイズ）
 */
export const memoizeHeavyComputation = <
  TArgs extends readonly unknown[],
  TReturn
>(
  fn: (...args: TArgs) => TReturn
